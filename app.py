...
# --- Lógica de Cálculos ---
class PlantoSaude:
    def __init__(self, peso: float, altura_cm: float, idade: int, sexo: str, objetivo: str, fator_atividade: float):
        self.peso = peso
    ...
    def calcular_tmb(1) -> float:
        if self.sexo == "Masculino":
            ...
            return (10 * self.peso) + (6.25 * self.altura_cm) - (5 * self.idade) - 161
    ...
    def gerar_plano_treino(self) -> dict:
        if self.objetivo in ["Emagrecimento", "Definição"]:
            ...
            return {
                ...
                "estrutura": "ABCDE ou Push/Pull/Legs com sobrecarga progressiva",
...
st.set_page_config(page_title="Gerador de Treino & Dieta", page_icon="🏋️‍♂️", layout="centered")

st.title("🏋️‍♂️ Gerador de Plano de Treino & Nutrição")
st.write("Insira os dados do usuário para gerar as recomendações personalizadas.")
...
with col1:
    ...
    altura = st.number_input("Altura (cm):", min_value=100.0, max_value=230.0, value=170.0, step=1.0)
    idade = st.number_input("Idade:", min_value=12, max_value=100, value=25, step=1)

with col2:
    sexo = st.selectbox("Sexo Biológico:", ["Masculino", "Feminino"])
    objetivo = st.selectbox("Objetivo Principal:", ["Hipertrofia", "Emagrecimento", "Definição", "Manutenção"])
    atividade = st.selectbox(
        "Nível de Atividade Física:",
        ["Sedentário", "Levemente Ativo (1-3 dias/sem)", "Moderadamente Ativo (3-5 dias/sem)", "Muito Ativo (6-7 dias/sem)"]
    )

# Mapeamento do Fator de Atividade
fator_map = {
    "Sedentário": 1.2,
...
st.write("")
if st.button("🚀 Gerar Plano Personalizado", type="primary", use_container_width=True):
    plano = PlantoSaude(
        peso=peso,
        altura_cm=altura,
        idade=idade,
